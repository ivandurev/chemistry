...
{
	var ret = "";
	ret += "<img src = 'Substances/"+substance[id].name+".jpg"+"' class = 'cardImg'><br><div id = 'iconContainer'><img class = 'cardRemove' onclick = 'javascript:removeReagent("+pl+")'src = 'x.png'></div>";
...
var updateBar = function()
{
	...
	var n = "<div id = 'pageText'>Page:&nbsp</div>";
	var p = 0;
	...
	{
		if(document.getElementById("currpageNum") != undefined) document.getElementById("currpageNum").style.top = "-11.8px";
		if(document.getElementById("pageNum") != undefined) document.getElementById("pageNum").style.top = "-11.8px";
		if(document.getElementById("pageText") != undefined) document.getElementById("pageText").style.top = "-9px";
	}
	updateXp();
}
...
{
	if(reagents.length >= 4) return alert("Too many substances!");
...
var addSubstance = function(cat, t) // funkciq koqto dobavq toku shto otklyucheno v-vo kum menuto
{
	...
	//console.log(cat, t);
	if(unlocked[t] == true || unlocked[t] != null) return;
...
var level = function(lvl) // diga levela
{
	if(unlocks[lvl] == undefined)
	{
		...
		lvl = Infinity;
		var str = "Congratulations! You have passed all levels! You still haven't done ";
		var c = 0;
		for(var i = 0; i < reaction.length; i ++)
		{
			if(reaction[i].xp == 0) c ++;
		}
		str += (reaction.length-c)+" reactions.";
		alert(str);
	}
	levelP = lvl;
	addMoney(5);
	$("#levelNotif").fadeOut();
	document.getElementById("levelNotif").innerHTML = "You're now level "+lvl+"!";
	$("#levelNotif").fadeIn("slow",function(){
		setTimeout(function()
		{
			$("#levelNotif").fadeOut("slow",function()
			{
				document.getElementById("levelNotif").innerHTML = "Level "+lvl;
				$("#levelNotif").fadeIn();
			});
		},1000);
...
{
	if(money < researchPrice) return alert("Sorry you don't have enough money to do research! ("+researchPrice+"&#128;)");
	addMoney(-researchPrice);
	//console.log(reagents);
	var elements = [];
	...
	if(equipMenuState == 1)
	{
		...
		for(var i = pageEquip*8; i < pageEquip*8+8 && (i < equipment.length); i ++)
		{
			...
			if(equipment[i].price == 0)
			{
				...
				n += "<img id = 'equipImage' src='tools/"+equipment[i].name+".jpg' style='"+(equipped[i]?"opacity:0;":"")+"'>";
				n += "</td>";
				c ++;
			}else{
...
var checkLevel = function()
{
	...
	{
		alert("Congratulations! You won! Score: "+money);
	}
	if(xp >= need[levelP])
	{
		level(levelP+1);
